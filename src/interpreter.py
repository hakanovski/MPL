    ...
    def execute(self, stmt: Stmt):
        ...
        if isinstance(stmt, Echo):
            ...
            try:
                ...
                print(f"🛡️ [CIRCLE] Protected against chaos: {e}")
            ...
            if val is not None:
                ...
                self.environment.assign(dummy_token, val)
                 ...
                 print(f"⚠️ [HEX] No 'value' parameter provided to hex '{stmt.target}'.")

        elif isinstance(stmt, Banish):
            self.environment.banish(stmt.target)
            print(f"🗑️ [BANISH] '{stmt.target}' cast into the void.")

        elif isinstance(stmt, Purge):
            self.environment.purge()
            print("🔥 [PURGE] Memory cleansed.")
